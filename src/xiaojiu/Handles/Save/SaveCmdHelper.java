...
public class SaveCmdHelper {
    public static void ShowTaskInfo(BasicSaveHandles task, Player player){
        ...
        if (task.getDelay()!=0) player.sendMessage(MessageHelper.InitMessage(ChatColor.LIGHT_PURPLE+"延迟时间:"+task.getDelay()));
        if (task.getTimerTime()!=0) player.sendMessage(MessageHelper.InitMessage(ChatColor.LIGHT_PURPLE+"执行时间间隔:"+task.getTimerTime()));
